create or replace package black as
  ...
  min_percent_ constant number := 80;
  ...
  -- Выделение строки
  procedure setChecked(site_ in number, id_ in number, list_id_ in number, checked_ in number);
...
create or replace package body black as
  ...
  begin
    ...
    WHEN NOT MATCHED THEN
      ...
      VALUES
        (s.source_id,
         (select max(id)+1 from BLACK_LIST),
         s.euentity_id,
         s.name,
         s.name_norm);
  ...
  begin
    ...
    USING (select siteid site,
                  ...
                  trim(REGEXP_REPLACE(upper(clientlastname),
                                      '[^A-Z А-Я]+',
                                      ' ')) last_name,
                  trim(REGEXP_REPLACE(upper(clientname),
                                      '[^A-Z А-Я]+',
                                      ' ')) first_name
  ...
  procedure startSearch is
    ...
    fio_    varchar2(256);
  begin
    if (getProcCount >= max_threads_) then return; end if;
      ...
      FOR r IN (select *
        ...
        BEGIN
          ...
          USING (select r.site site, r.id id, id list_id, jws
                   from (SELECT id,
                                UTL_MATCH.jaro_winkler_similarity(fio_,
                                                                  name_norm) AS jws
                           FROM BLACK_LIST)
  ...
  begin
    for i in 1 .. max_threads_ loop
      dbms_scheduler.create_job(job_name   => 'B2CheckJob#'||i,
                                ...
                                enabled    => TRUE,
                                auto_drop  => TRUE,
                                comments   => 'B2CheckJob#'||i||'. Поиск контрагентов в черных списках');
    end loop;
  EXCEPTION
    WHEN OTHERS THEN
  ...
  --
  procedure setChecked(site_ in number, id_ in number, list_id_ in number, checked_ in number) is
  begin
    update black_result set checked = checked_, checked_by = user where site = site_ and id = id_ and list_id = list_id_;
    commit;
  end;
  
  -- Починялка нумерации черного списка
  procedure fixList is
    l_cursor SYS_REFCURSOR;
    str  varchar2(1000);
    i number := 159153;
    id_ number;
  begin
    str := 'select id from BLACK_LIST where id >= 18812258 order by id';
    OPEN l_cursor FOR str;
    LOOP
      FETCH l_cursor
        INTO id_;
    ...
    END LOOP;
    CLOSE l_cursor;
    --update BLACK_CONTR set max_id = i where max_id > 159153;
    --commit;
  end;
